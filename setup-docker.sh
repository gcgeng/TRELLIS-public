...
case $PLATFORM in
    ...
    hip)
        ...
        echo "[SYSTEM] Installing PyTorch 2.4.1 for HIP ($PYTORCH_VERSION -> 2.4.1+rocm6.1)"
            ...
            mkdir -p /tmp/gengchen/extensions
            sudo cp /opt/rocm/share/amd_smi /tmp/gengchen/extensions/amd_smi -r
            cd /tmp/gengchen/extensions/amd_smi
            sudo chmod -R 777 .
            pip install .
            cd $WORKDIR
            PYTORCH_VERSION=$(python -c "import torch; print(torch.__version__)")
...
if [ "$TRAIN" = true ] ; then
    pip install tensorboard pandas lpips webdataset mosaicml-streaming zstandard fsspec s3fs
...
if [ "$FLASHATTN" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        ...
        mkdir -p /tmp/gengchen/extensions
        git clone --recursive https://github.com/ROCm/flash-attention.git /tmp/gengchen/extensions/flash-attention
        cd /tmp/gengchen/extensions/flash-attention
...
if [ "$NVDIFFRAST" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/gengchen/extensions
        git clone https://github.com/NVlabs/nvdiffrast.git /tmp/gengchen/extensions/nvdiffrast
        pip install /tmp/gengchen/extensions/nvdiffrast
    else
...
if [ "$DIFFOCTREERAST" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/gengchen/extensions
        git clone --recurse-submodules https://github.com/JeffreyXiang/diffoctreerast.git /tmp/gengchen/extensions/diffoctreerast
        pip install /tmp/gengchen/extensions/diffoctreerast
    else
...
if [ "$MIPGAUSSIAN" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/gengchen/extensions
        git clone https://github.com/autonomousvision/mip-splatting.git /tmp/gengchen/extensions/mip-splatting
        pip install /tmp/gengchen/extensions/mip-splatting/submodules/diff-gaussian-rasterization/
    else
...
if [ "$VOX2SEQ" = true ] ; then
    if [ "$PLATFORM" = "cuda" ] ; then
        mkdir -p /tmp/gengchen/extensions
        cp -r extensions/vox2seq /tmp/gengchen/extensions/vox2seq
        pip install /tmp/gengchen/extensions/vox2seq
    else
